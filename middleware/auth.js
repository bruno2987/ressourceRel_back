...
exports.mod = (req,res,next) => {
    UsersModel.findOne({_id: req.headers.userid}, {habilitation:1, _id:0}, function(err,datas){
        if(datas.habilitation == 'mod' || datas.habilitation == 'admin' || datas.habilitation == 'superadmin'){
            console.log('connexion mod')
            authent(req,res,next)
        } else{
            return res.status(401).json({ message: "vous n'avez pas les habilitations" });
        }
    })
}
...
exports.admin = (req,res,next) => {
    UsersModel.findOne({_id: req.headers.userid}, {habilitation:1, _id:0}, function(err,datas){
        if(datas.habilitation == 'admin'|| datas.habilitation == 'superadmin'){
...
exports.superadmin = (req,res,next) => {
    UsersModel.findOne({_id: req.headers.userid}, {habilitation:1, _id:0}, function(err,datas){
        if(datas.habilitation == 'superadmin'){
